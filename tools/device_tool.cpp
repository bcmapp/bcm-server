...
void generateDeviceRequestInfo(uint32_t deviceId, uint32_t expiretime)
{
    ...
    DeviceLoginReqInfo info;
    info.set_publickey(encodedPubkeyWithDJBType);
    info.set_nounce(token);
    info.set_signature(encodedSignature);
    info.set_deviceid(deviceId);
    info.set_devicename("CCL de Macbook");
    info.set_expiretime(nowInSec() + expiretime);

    auto content = info.SerializeAsString();

    struct timeval timeout = {1, 500000};
    auto redisContext = redisConnectWithTimeout("127.0.0.1", 4011, timeout);
    if ((redisContext == nullptr) || (redisContext->err != REDIS_OK)) {
        std::cout << "failed to connect redis (  error " << redisContext->errstr << ")" << std::endl;
        exit(1);
    }

    redisReply* pReply = nullptr;
    pReply = static_cast<redisReply*>(redisCommand(redisContext, "SET %b %b EX %d", uid.c_str(), uid.size(), content.c_str(), content.size(), expiretime));
    if (nullptr == pReply || pReply->type == REDIS_REPLY_ERROR) {
        std::cout << "set redis error" << std::endl;
    }

    std::cout << "Device request login Generate Finish: " << std::endl;
    std::cout << "\tuid:                    " << uid << std::endl;
...
int main(int argc, char** argv)
{
    ...
    cmdOpt.add_options()
        ...
        ("deviceid,i", po::value<uint32_t>(&deviceId), "device id")
        ("expire,x", po::value<uint32_t>(&expireTime), "expiretime")
    ...
    if (doRequest) {
        generateDeviceRequestInfo(deviceId, expireTime);
    }
